...
int db_find (int table_id, int64_t key, char * ret_val) {
	record * r = find(table_id, root, key, true);
        if (r == NULL)
                return -1;
        else {
...
void print_tree_disk(node * root ) {
    ...
    while( queue != NULL ) {
        ...
        for (i = 0; i < n->num_keys; i++) {            
            ...
            if (!n->is_leaf) {r = find(table_id,root, n->keys[i], 0);}
...
void find_and_print(int table_id, node * root, int key, bool verbose) {
    record * r = find(table_id, root, key, verbose);
    if (r != NULL)
 ...
 */
node * find_leaf(int table_id, node * root, int key, bool verbose ) {
    int i = 0;
    int frame_id = 0;
 ...
 * a key refers.
 */
record * find(int table_id, node * root, int key, bool verbose ) {
    int i = 0;
    node * c = find_leaf(table_id, root, key, verbose );
...
node * insert(int table_id, node * root, int key, char * value ) { //2020-10-04 changed 3rd parameter to char pointer
    ...
    if (find(table_id,root, key, false) != NULL){
		record * temp = find(table_id,root, key, true);
