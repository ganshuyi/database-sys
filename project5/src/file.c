...
int open_table (char *pathname) {
	...
	last_table_id++;
	//table_id_ls[last_table_id] = bpt_filename;
	strcpy(table_id_ls[last_table_id],bpt_filename); //strcopy(destination, src)
	...
	if (!fptr) {//if file does not exist
		...
		build_bpt(last_table_id,pathname);
...
int db_update(int table_id, int64_t key, char* values, int trx_id) {
	record * r = find(table_id, root, key, false, trx_id);
	if (r != NULL) {  //if found
		int stat = add_to_trx_arr(table_id,key,trx_id,1,'U',r->value,values); //store ori value and new value, for rollback, if needed
		if (stat) {
			printf("found key: %ld, value: %s\n",key,r->value);
			strcpy(r->value,values); //strcopy(destination, src)
...
int add_to_trx_arr(int table_id, int64_t key, int trx_id, int lock_mode, char act_name, char * ori_val, char * new_val) {
	...
	//printf("add_to_trx tbl:%d, key:%ld, trx:%d, act:%c\n",table_id,key,trx_id,act_name);
    ...
    if (trec == NULL) {printf("Unable to find Transaction Record for trx_id:%d\n", trx_id);}
		...
		if (trec->actions == NULL) {printf("No action for Transaction Record for trx_id:%d\n", trx_id);}
			...
			while (xa != NULL) { //release all
				printf("Tbl:%d, Key: %ld, A:%c\n", xa->table_id,xa->key,xa->act_name);
    			xa = xa->next; //move to next in the list
			}//while
		} //else
    }//else
